...
const tipoMap: Record<string, "sol" | "grau" | "armacao"> = {
  Sol: "sol", Grau: "grau", Armacao: "armacao",
};
          ...
          <Link key={p.id} href={`/produto/${p.id}`} style={{ textDecoration: "none", color: "inherit" }}>
            ...
            >
              <div style={{ height: 200, background: "#f0ebe2", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                ...
                <div style={{ position: "absolute", top: "1rem", left: "1rem", background: "#2c1f0e", color: "#faf7f2", fontSize: 9, letterSpacing: "0.1em", textTransform: "uppercase", padding: "4px 10px" }}>{p.badge}</div>
              </div>
              <div style={{ padding: "1.25rem" }}>
                <div style={{ fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: "#b8914a", marginBottom: 6 }}>{p.marca}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: "1rem", borderTop: "0.5px solid #e0d5c5" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#8c7b68", fontWeight: 300 }}>{p.parcelas}x de R$ {Math.ceil(p.preco / p.parcelas).toLocaleString("pt-BR")}</div>
                  </div>
